    ...
    tokio::spawn(async move {
        loop {
            ...
            if let Err(e) = recv.read_exact(&mut hdr).await {
                ...
                eprintln!("[DEAD] spawn_reader: peer={} read hdr error: {e:?}", peer_id);
...
pub async fn dial_workers(
    ...
    loop {
        tokio::select! {
            result = futs.next() => {
                match result {
                    Some(Ok((conn, send, recv))) => {
                        workers.push(make_transport(
                            peer_id.clone(), conn, send, recv, tx.clone(), false,
                        ));
        ...
        tokio::select! {
            result = futs.next() => {
                match result {
                    Some(Ok((conn, send, recv))) => {
                        workers.push(make_transport(
                            peer_id.clone(), conn, send, recv, tx.clone(), true,
                        ));
                        if workers.len() >= count { break; }
                    }
                    Some(Err(e)) => {
