...
// new session can ride THAT link instead, skipping establishment entirely. The
// daemon exposes a unix-domain socket; a sibling process connects, names a peer
// and a remote port, and on success the socket becomes a raw byte pipe for one
// L2 stream the daemon opens over its warm link.
//
...
/// else `~/.config/filament`. Mirrors `devices_path()` / `pidfile()`.
...
/// Read a single newline-terminated line, byte at a time, so we never consume the
/// raw stream bytes that follow the JSON line. Lines are tiny, so this is cheap.
...
/// Try to open an L2 stream to `peer:rport` THROUGH a local daemon's warm link.
/// Returns the connected socket (positioned for raw bytes) on success, or `None`
/// if there is no daemon, no warm link, or any protocol error, so the caller
/// falls back to a fresh establish. Never errors: a miss is just `None`.
...
pub async fn try_open_at(path: &std::path::Path, peer: &str, rport: u16) -> Option<UnixStream> {
...
/// A parsed open request handed to the daemon's event loop, which owns the link
/// table and the per-peer muxes. The loop resolves `peer` to a warm link and then
...
impl Req {
    /// Confirm the stream is opening; returns the socket for the bridge to drive.
    ...
    /// Decline (no warm link / not permitted); the client falls back to a fresh
    /// establish. Best-effort; the socket drops on return.
...
/// Bind the control socket and forward each parsed request to `tx` (the daemon
/// event loop). Removes a stale socket file first (a previous daemon's leftover);
/// `daemon_alive()` already guards against two live daemons. Sets mode 0600.
/// Returns only on a fatal bind error; otherwise loops for the daemon's life.
...
pub async fn serve_at(path: PathBuf, tx: mpsc::UnboundedSender<Req>) -> Result<()> {
    ...
    loop {
        ...
        let tx = tx.clone();
        // Read the request line off the loop so a slow/garbage client cannot stall
...
mod tests {
    ...
    fn control_sock_path_ends_with_socket_name() {
        // The resolver mirrors devices_path()/pidfile(); just assert the file name
        // (no global-env mutation, so this is parallel-test safe).
        assert!(control_sock_path().ends_with("control.sock"));
    }

    #[tokio::test]
    async fn request_line_round_trips_and_pipes_raw_bytes() {
        // End-to-end of the wire protocol (no link involved): a client connects,
        // sends the request line, the daemon side parses it into a Req, accepts,
        // and then raw bytes flow both ways on the same socket.
        ...
        // Give the listener a moment to bind, then drive a client.
