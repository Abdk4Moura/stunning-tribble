"""Runtime configuration for the Quickshare backend.
...
SIGNALING = os.environ.get("QS_SIGNALING", "socketio").lower()

# Secret used to derive a stable, non-reversible default room name per network.
SECRET = os.environ.get("QS_SECRET", "quickshare-dev-secret")

PORT = int(os.environ.get("PORT", 5000))

# Allow the Vite dev server origin to open the websocket during development.
CORS_ALLOWED_ORIGINS = os.environ.get("QS_CORS_ORIGINS", "*")
...
def _ice_servers():
    raw = os.environ.get("QS_ICE_SERVERS")
    if not raw:
        return DEFAULT_ICE
    try:
...
def _firebase_web_config():
    ...
    raw = os.environ.get("QS_FIREBASE_CONFIG")
    if not raw:
        return None
    try:
        return json.loads(raw)
...
def public_config() -> dict:
    ...
    return {
        ...
        "chunkSize": int(os.environ.get("QS_CHUNK_SIZE", 64 * 1024)),
    }
