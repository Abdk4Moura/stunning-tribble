...
pub fn managed_key_path(config_dir: &Path) -> PathBuf {
...
pub fn ensure_managed_key(config_dir: &Path) -> Result<String> {
    let key = managed_key_path(config_dir);
    ...
    if let Err(e) = SecretFile::restrict(&key) {
        if created_this_call {
...
pub fn install_authorized_key(device: &str, pubkey: &str) -> Result<()> {
    ...
    SecretFile::write_str(&path, &kept)
...
fn bootstrap_cache_path(config_dir: &Path) -> PathBuf {
...
pub fn host_pinned(config_dir: &Path, dest_token: &str) -> bool {
    std::fs::read_to_string(known_hosts_path(config_dir))
...
pub fn bootstrap_cache_get(config_dir: &Path, device: &str) -> Option<Option<String>> {
    let raw = std::fs::read_to_string(bootstrap_cache_path(config_dir)).ok()?;
...
pub fn bootstrap_cache_clear(config_dir: &Path, device: &str) {
    let path = bootstrap_cache_path(config_dir);
